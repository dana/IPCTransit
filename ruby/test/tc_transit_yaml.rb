...
class TestIPCTransit < Test::Unit::TestCase
    def test_yaml_typical
        drain_test_queue()
        IPCTransit.send('message' => { 'foo' => 'bar' }, 'e' => 'yaml', 'qname' => 'tr_dist_test_qname')
    ...
    def test_yaml_wire_raw
        drain_test_queue()
        IPCTransit.send('message' => { 'foo' => 'bar' }, 'qname' => 'tr_dist_test_qname', 'e' => 'yaml', 'c' => 'none')
    ...
    def test_yaml_message_meta
        ...
        ret = IPCTransit.receive('qname' => 'tr_dist_test_qname', 'e' => 'yaml', 'nowait' => 1)
        assert(ret, 'IPCTransit.receive returned true')
        assert_equal(ret['foo'], 'bar')
